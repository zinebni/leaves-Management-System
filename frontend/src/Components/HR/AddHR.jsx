...
export default function AddHR() {
  ...
  const add = async () => {
    ...
    try{
      setError({});
      const res = await api.post('/api/auth/rhRegister', rh);
      setLastName('');
      setFirstName('');
      setEmail('');
      setContact('');
  ...
  return (
   <div className={`flex justify-center items-center w-full`}>
        ...
        <div className='flex justify-end w-full mt-5'>
          <button className='text-base sm:text-[17px] font-semibold bg-blue-700 dark:bg-blue-900/90 dark:hover:bg-blue-800/80 w-full sm:w-xs py-2 text-white rounded-md sm:rounded-lg mb-2 cursor-pointer hover:bg-blue-600'
            onClick={add}
          >
            {t('add_rh')}
          </button>
